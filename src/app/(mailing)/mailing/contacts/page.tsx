...
import { Search, User, MoreHorizontal, Contact } from "lucide-react"
...
export default function MailingContactsPage() {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 lg:px-0">
        <div className="relative">
          ...
          <p className="text-muted-foreground text-sm mt-2 font-medium">
            {contacts.length}{features.max_contacts !== null ? ` / ${features.max_contacts}` : ""} אנשי קשר
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-border/50 overflow-hidden bg-card">
            ...
            <TableBody>
              {contacts.map((c) => (
                ...
                >
                  ...
                  <TableCell className="text-muted-foreground">
                    {can("contacts", "view_source") ? (c.source ?? "—") : "•••"}
                  </TableCell>
          ...
          {contacts.length === 0 && (
            <div className="py-12 text-center text-muted-foreground">
              אין אנשי קשר. הוסף איש קשר חדש כדי להתחיל.
            </div>
          )}
        </div>
      )}
